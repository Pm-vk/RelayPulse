...
export const AuthGuard: React.FC<AuthGuardProps> = ({ children, requireAdmin = false }) => {
  ...
  if (isLoading || (!user && typeof window !== 'undefined' && localStorage.getItem('accessToken'))) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#f6f5f2] text-[#18181b]">
        <div className="flex flex-col items-center gap-3">
          <svg className="h-8 w-8 animate-spin text-[#f05228]" fill="none" viewBox="0 0 24 24">
          ...
          <p className="text-xs font-semibold tracking-wider text-zinc-500">Authenticating Session...</p>
        </div>
      </div>
    );
  }
